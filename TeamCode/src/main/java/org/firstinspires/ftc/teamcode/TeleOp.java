...
public class TeleOp extends LinearOpMode {
    ...
    // SHOOTER
    double startTime;
    double closeGate = 0.4;
    double openGate = 0.235;
    ...
    public void runOpMode() {
        ...
        if (Red) {
            follower.setStartingPose(new Pose(88, 9, Math.toRadians(90))); // RED
        } else {
            follower.setStartingPose(new Pose(88, 9, Math.toRadians(90))); // BLUE
        }
...
//            }
            ...
            if (gamepad1.dpadUpWasPressed()) {
                ShooterCalc.adjustDistance("tooFar");
            } else if (gamepad1.dpadDownWasPressed()){
                ShooterCalc.adjustDistance("tooClose");
            }
            // shooting
            if (!Scheduler.isRunning(ShootRoutine)) {
                ...
                if (gamepad1.leftBumperWasPressed()) {
                    if (Red) {
                        follower.setPose(new Pose(8, 9, Math.toRadians(0)));
                    } else {
                        follower.setPose(new Pose(136, 9, Math.toRadians(0)));
                    }
                }
    ...
    public Command ShootRoutine = Command.build()
            ...
            .setExecute(() -> {
                Shooter.setVelocity(ShooterVel);
                if (Shooter.getVelocity() >= ShooterVel-40 && Shooter.getVelocity() <= ShooterVel+40) {
                ...
                if (System.currentTimeMillis() - startTime > intervaltime) {
                    IntakeControl.setPower(0);
                } else if (System.currentTimeMillis() - startTime > intervaltime*2) {
                    IntakeControl.setPower(1);
                    startTime = System.currentTimeMillis();
                    shot += 1;
                }
